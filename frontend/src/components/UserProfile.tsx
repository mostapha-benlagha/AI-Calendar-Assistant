  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center space-x-1 sm:space-x-2 p-1.5 sm:p-2 rounded-lg hover:bg-slate-100/80 dark:hover:bg-slate-700/80 transition-colors"
      >
        {user.picture ? (
          <img
            src={user.picture}
            alt={user.name}
            className="w-7 h-7 sm:w-8 sm:h-8 rounded-full ring-2 ring-slate-200 dark:ring-slate-600"
          />
        ) : (
          <div className="w-7 h-7 sm:w-8 sm:h-8 bg-indigo-600 rounded-full flex items-center justify-center ring-2 ring-slate-200 dark:ring-slate-600">
      ...
      {isOpen && (
        <>
          ...
          {/* Dropdown */}
          <div className="absolute right-0 mt-2 w-72 sm:w-80 md:w-96 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 z-[9999] max-w-[calc(100vw-2rem)]">
            <div className="p-3 sm:p-4">
              ...
              <div className="flex items-center space-x-2 sm:space-x-3 mb-3 sm:mb-4">
                {user.picture ? (
                  <img
                    src={user.picture}
                    alt={user.name}
                    className="w-10 h-10 sm:w-12 sm:h-12 rounded-full ring-2 ring-slate-200 dark:ring-slate-600 flex-shrink-0"
                  />
                ) : (
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-indigo-600 rounded-full flex items-center justify-center ring-2 ring-slate-200 dark:ring-slate-600 flex-shrink-0">
                    <User className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
                  </div>
                )}
                <div className="min-w-0 flex-1">
                  <h3 className="font-semibold text-slate-900 dark:text-slate-100 truncate text-sm sm:text-base">{user.name}</h3>
                  <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 truncate">{user.email}</p>
                </div>
              </div>

              {/* Calendar Status */}
              <div className="border-t border-slate-200 dark:border-slate-700 pt-3 sm:pt-4 mb-3 sm:mb-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  {calendarStatus.isLoading ? (
                    <div className="animate-spin rounded-full h-3.5 w-3.5 sm:h-4 sm:w-4 border-b-2 border-indigo-600"></div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-2 sm:mb-3 leading-relaxed">
                  {calendarStatus.isConnected
              ...
              {/* Actions */}
              <div className="border-t border-slate-200 dark:border-slate-700 pt-3 sm:pt-4 space-y-1.5 sm:space-y-2">
                <button
                  onClick={() => {
                    setIsOpen(false);
                    // You could add settings functionality here
                  }}
                  className="w-full flex items-center space-x-2 text-xs sm:text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 py-2 px-3 rounded transition-colors"
                >
                ...
                <button
                  ...
                  className="w-full flex items-center space-x-2 text-xs sm:text-sm text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 py-2 px-3 rounded transition-colors"
                >
                  <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  <span className="hidden sm:inline">Debug Auth & Check Calendar</span>
                  <span className="sm:hidden">Debug & Check</span>
                </button>
                
                <button
                  onClick={handleLogout}
                  className="w-full flex items-center space-x-2 text-xs sm:text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 py-2 px-3 rounded transition-colors"
                >

...
class CalendarService {
  ...
  async createEvent(eventData, user = null) {
    try {
      ...
      if (user && user.calendarKeys.accessToken) {
        ...
        throw new Error('Calendar not authenticated. Please complete OAuth flow first.');
      }
      
      const event = this.formatEventData(eventData);
      const calendarId = user?.calendarKeys.calendarId || config.google.calendarId;
    ...
    try {
      ...
      if (identifier && identifier.trim() !== '') {
        ...
        matchingEvents = events.filter(event => {
          ...
          const significantWords = searchWords.filter(word => word.length >= 3);
          if (significantWords.length > 0) {
            const anySignificantWordFound = significantWords.some(word => 
              title.includes(word) || description.includes(word)
            );
   ...
   */
  async createFollowupEvent(originalEventId, followupDays, customData = {}, user = null) {
    try {
      ...
      const followupTime = new Date(startTime.getTime() + followupDays * 24 * 60 * 60 * 1000);
      ...
      const followupEventData = {
        ...
        description: customData.description || `Follow-up meeting for: ${event.summary}`,
        attendees: customData.attendees || event.attendees?.map(a => a.email) || []
      };
  ...
  formatEventData(eventData) {
    const { title, date, time, end, duration, location, attendees, description } = eventData;
    ...
    if (!eventTitle || eventTitle.trim() === '') {
      eventTitle = `Meeting on ${startDateTime.format('MMM DD, YYYY')} at ${startDateTime.format('HH:mm')}`;
    }
  ...
  mergeEventData(existingEvent, updateData) {
    ...
    if (updateData.date && updateData.time) {
      const startDateTime = moment(`${updateData.date} ${updateData.time}`, 'YYYY-MM-DD HH:mm');
      merged.start = {
        dateTime: startDateTime.toISOString(),
        timeZone: 'UTC'
      };
      ...
      if (updateData.end) {
        const endDateTime = moment(`${updateData.date} ${updateData.end}`, 'YYYY-MM-DD HH:mm');
        merged.end = {
          dateTime: endDateTime.toISOString(),
          timeZone: 'UTC'
        };
      } else if (updateData.duration) {
        const endDateTime = startDateTime.clone().add(updateData.duration, 'minutes');
        merged.end = {
          dateTime: endDateTime.toISOString(),
          timeZone: 'UTC'
        };
        ...
        const endDateTime = startDateTime.clone().add(originalDuration, 'minutes');
        merged.end = {
          dateTime: endDateTime.toISOString(),
          timeZone: 'UTC'
        };
    ...
    if (updateData.attendees) {
      ...
      if (validAttendees.length > 0) {
        ...
        // If no valid emails, add attendees as display names in description
        const attendeeNames = updateData.attendees.filter(name => name && name.trim());
        if (attendeeNames.length > 0) {
          const attendeeText = attendeeNames.join(', ');
